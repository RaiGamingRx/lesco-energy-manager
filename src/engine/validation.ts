...
export function validateMeterReading(
  ...
  const meter = context?.meter || {
    id: existingReadings[0]?.meterId || 'preview-meter', householdId: household.id, name: 'Preview meter',
  ...
  const result = validateReadingMutation({
    id: '__preview__', cycleId: cycle.id, meterId: meter.id, householdId: household.id,
